...
dwf_consum_dom <- country_i_dwf_consump %>% 
  filter(eez_iso3c == country_iso3c)
  # caught in a different 
dwf_consum_frgn <- country_i_dwf_consump %>% 
  filter(eez_iso3c != country_iso3c)
...
### DWF source EEZ --> Producer --> Consumer flows (2019)
...
purrr::map(top_6_sp$sciname, ~ plot_sankey(dwf_frgn_consum_sankey %>% 
                                             filter(.x == sciname),
                                           cols = c("eez_name", 
                                                    "producer_iso3c_name",
                                                    "consumer_iso3c_name"),
...
```{r}
# dwf_consum_dom is  Focal country consumption of catch in its own waters
# could be domestic catch or catch by a different nation
# need fill to be producer nation
# function not updated yet

dwf_consum_dom_plot <- dwf_consum_dom %>% 
  filter(dwf == "domestic")

plot_ts(dwf_consum_dom_plot,
...
### Fish consumed by `r country_name`, caught in its waters by a differnt country

```{r}

dwf_consum_dom_plot2 <- dwf_consum_dom %>% 
  filter(dwf == "foreign")

plot_ts(dwf_consum_dom_plot2,
        artis_var = "producer_iso3c",
        prop_flow_cutoff = 0.05,
        plot.type = "stacked",
        legend.title = ""
...
### Fish consumed by `r country_name`, caught by `r country_name` in a different EEZ
