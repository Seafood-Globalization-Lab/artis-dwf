...
artis_eez_i <- params$artis_eez_i

# df of focal country - dwf - all years
artis_eez_i_dwf <- artis_eez_i %>% 
  filter(dwf == "foreign")
...
artis_eez_i_dwf_5 <- artis_eez_i_dwf %>% 
...
exploreARTIS::plot_bar(artis_eez_i_dwf_5,
                       bar_group ="sciname")
...
Sum across last 5 years of available data (`r paste(last_5_yrs, sep = ", ")`)
...
```{r, eval=FALSE}
top_sp_consump <- artis_eez_i_dwf_5 %>% 
  filter()
unique(top_6$sciname)
exploreARTIS::plot_ts(artis_eez_i_dwf_5,
                      artis_var = "producer_iso3c",
                      #plot.title = "DWF Catch by EEZ",
                      prop_flow_cutoff = 0.05,
                      plot.type = "stacked",
