...
purrr::map(top_6_sp$sciname, 
           ~ plot_sankey(dwf_frgn_consum_sankey %>% filter(.x == sciname),
                         cols = c("eez_name", 
                                  "producer_iso3c_name",
                                  "consumer_iso3c_name"),
                         plot.title = .x))
...
# other option for renaming - requires adding facet_wrap argument in plot_ts
# facet_names <- as_labeller(c(
#   "frgn_dom_prod" = "Foreign domestic\nproduction",
#   "frgn_dwf_prod" = "Foreign DWF production",
#   "dom_dwf_prod" = glue("{country_iso3c} DWF production")
# )
