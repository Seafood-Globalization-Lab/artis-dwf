...
This report provides an analysis of `r country_name`'s distant water fishing. This pdf uses Tufte style formatting and separate `.R` files to pull in country specific markdown annotations. LaTeX formatting underlying this often makes figures float away to locations where spacing is optimal -- write out text first then place figures as a final tweak.
...
```{r prct-dwf, fig.cap= "Percent DWF of total catch"}
# calc total catch
...
```{r fig-bar-dwf-species, eval=TRUE, fig.cap=paste0("Top species caught via DWF. Sum for the last 5 years of available data (", paste(last_5_yrs, collapse = ", "), ")")}
...
```{r figs-dwf-species-sankeys, echo=FALSE, results='hide', fig.keep='all', fig.cap=paste0(top_6_sp$common_name, ": caught via DWF by ", country_name, " (source EEZ --> producer --> consumer)")}
...
```{r  ,out.width="100%", fig.width=8.57,fig.cap= paste0(country_name, " consumption of catch from its own waters")}
# 
...
```{r fig-facet-consump, out.width="100%", fig.width=8.57, fig.cap=paste0("Fish consumed by ", country_name, " from another country's waters")}
#
