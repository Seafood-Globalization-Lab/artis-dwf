...
```{r fig-prct-eez-frgn ,fig.cap= glue::glue("Percent of {country_name} total catch that is distant water fishing for most recent available data.")}
# new figure --
...
```{r fig-sankey-eez-all, fig.cap= paste0("Flows of all fishing activity occuring in ", country_name,"'s EEZ waters. Total for all years of available data (", paste(unique(country_i_eez_fishing$year), collapse = ", "),").")}
# New Figure --
...
```{r figs-sankeys-eez-frgn-species, results='hide', fig.keep='all', fig.cap=paste0("Total DWF catch in ", country_name, "'s EEZ waters for ", top_6_sp_eez_frgn$common_name," across all years of available data (", paste(unique(country_i_eez_fishing$year), collapse = ", "),").")}
...
```{r fig-bar-dwf-species, eval= nrow(dwf_frgn) > 0, fig.cap=paste0("Top species caught via distant water fishing by tons of live weight. Sum for the most recent available data (", paste(unique(dwf_frgn$year), collapse = ", "), ")")}
...
```{r fig-sankey-dwf-all, fig.cap= paste0("All flows of ", country_name, "'s DWF activity. Total for all years of available data (", paste(unique(country_i_eez_fishing$year), collapse = ", "),").")}
# New Figure --
...
```{r figs-sankeys-dwf-species, results='hide', fig.keep='all', fig.cap=paste0(country_name, "'s total DWF catch of ", top_6_sp_dwf$common_name, " for the most recent available data (", paste(unique(dwf_frgn$year), collapse = ", "), ")")}
...
```{r fig-consump, fig.cap= paste0(country_name, " consumption of catch from its own waters. Domestic production referes to catch ", country_name, "produces and Foreign fleet production is a foreign country's catch in ", country_name, " water. Note y-axes may not be same scale and years in data may not be consistent.")}

# out.width="100%", fig.width=8.57,
...
```{r fig-sankey-consump-all, fig.cap= paste0("All flows of ", country_name, "'s DWF consumption. Total for all years of available data (", paste(unique(consum_dwf$year), collapse = ", "),").")}
# New Figure
