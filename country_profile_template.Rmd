...
This report provides an analysis of `r country_name`'s distant water fishing (DWF). This pdf uses Tufte style formatting and separate `.R` files to pull in country specific markdown annotations (in `./annotations/` directory). `LaTeX` formatting underlying this document often makes figures float away to locations where spacing is optimal -- write out text first then place figures as a final tweak.
...
```{r fig-prct-dwf, fig.cap= "Percent DWF of total catch"}
# calc total catch
...
```{r fig-line-dwf-eez, fig.cap="DWF catch by EEZ and year"}
...
```{r fig-bar-dwf-species, eval= nrow(dwf_frgn) > 0, fig.cap=paste0("Top species caught via DWF. Sum for the last 5 years of available data (", paste(last_5_yrs, collapse = ", "), ")")}
...
```{r fig-stacked-consum-dwf, fig.cap=paste0("DWF exports by ", country_name, " in the most recent 5 years of data (", paste(last_5_yrs, collapse = ", "), ") and not including NAs")}
...
knitr::kable(top_6_sp %>% 
            ...
            col.names = c("Common name", 
                          "Scientific name", 
                          "live weight (tons)"),
            caption = paste0("Top DWF Species caught by ", country_name, ". Sum of most recent 5 years of data (", paste(last_5_yrs, collapse = ", "), ")")
            )
```

```{r, 1-table-dwf-species, results='asis'}
```

```{r, 5-10-figs-dwf-species-sankeys, results='asis'}
```

```{r figs-dwf-species-sankeys, echo=FALSE, results='hide', fig.keep='all', fig.pos='h', fig.cap=paste0(top_6_sp$common_name, ": caught via DWF by ", country_name, " (source EEZ --> producer --> consumer)")}
...
```{r fig-consump, out.width="100%", fig.width=8.57,fig.cap= paste0(country_name, " consumption of catch from its own waters. Note y-axes may not be same scale.")}
...
```{r, fig-bar-top-sp-consump, fig.cap=paste0("Top species consumed by ", country_name, " caught via DWF by ", country_name, " and other countries. Sum for the last 5 years of data (", paste0(last_5_yrs, collapse = ", "), ").")}
...
knitr::kable(top_6_sp %>% 
            ...
            caption = paste0("Top species consumed by ", country_name, " caught via DWF by ", country_name, " and other countries. Sum for the last 5 years of data (", paste0(last_5_yrs, collapse = ", "), ")."))
```

```{r, 2-table-top-sp-consumed, results='asis'}
```

```{r, 14-19-figs-sankey-consump, results='asis'}
```

```{r figs-sankey-consump, echo=FALSE, results='hide', fig.keep='all', fig.cap=paste0(top_6_sp$common_name, ": ", country_name, " consumption of distant water fishing products (source EEZ --> producer --> consumer)")}

purrr::map(top_6_sp$common_name, 
           ~ plot_sankey(consum_dwf_5yr %>% 
                           filter(.x == common_name),
                         cols = c("eez_name", 
                                  "producer_iso3c_name",
                                  "consumer_iso3c_name"),
