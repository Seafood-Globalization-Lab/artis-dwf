...
library(exploreARTIS) # custom ARTIS package 
...
# why is this only displaying 10 out of the 17? too small to display?
(fig_catch_eez <- catch_eez_yr %>% 
  ggplot(aes(year, catch, fill = catch_artis_country_name)) +
    geom_area() +
    labs(x = "",
         y = "Catch Live Weight (tons)",
         fill = "Catch EEZ") +
    theme_bw() +
     scale_fill_manual(values = artis_palette(10))
)


# exploreARTIS::plot_ts(catch_eez,
#                       artis_var = 
#                       weight = "live",
#                       plot.type = "line",
#                       )
