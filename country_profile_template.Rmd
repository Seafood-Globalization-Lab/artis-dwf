...
# Sankey DWF chunk option template named "dwf_sankey_opts"
knitr::opts_template$set(
  dwf_sankey_opts = list(
    echo = FALSE,
    warning = FALSE,
    message = FALSE
  )
...
## 1) DWF Activities by `r country_name`
...
## 2) `r country_name` exports of DWF products
...
```{r figs-sankey-consump, opts.label='sankey_consump_opts'}

results <- purrr::map(top_6_sp_consum$common_name, 
...
```{r ,opts.label='sankey_consump_opts'}
print(results[1])
```
...
## 4) DWF in `r country_name` EEZ waters by other countries
